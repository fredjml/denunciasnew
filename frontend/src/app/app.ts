...
type WizardStep =
  ...
  | 'REVISAO'
  | 'CONFIRMACAO';
type WizardTab = 'PORTAL' | 'ACOLHIMENTO' | 'RELATO' | 'EMPRESA' | 'REVISAO';
...
export class App {
  private readonly currentStepState = createPersistedSignal<WizardStep>('wizard_step', 'PORTAL');
  ...
  protected startReport(choice: AcolhimentoChoice): void {
    if (choice === 'OUVIDORIA') {
      window.open(MPT_LINKS.ouvidoria, '_blank', 'noopener,noreferrer');
      return;
    }
    this.goTo('RELATO');
  }
