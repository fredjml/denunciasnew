...
async function receberDenuncia(req, res, next) {
  try {
    ...
    // Encaminhar para API interna do MPT (se configurada)
    ...
    if (mptApiUrl) {
      try {
        ...
        // Log do erro mas não falha o usuário — armazenar localmente se necessário
        console.error(`[ERRO API MPT] Protocolo: ${protocolo}`, apiError.message);
        // Em produção, aqui você poderia salvar em banco de dados como fallback
      }
    } else {
      console.warn(`[AVISO] MPT_API_URL não configurada. Denúncia ${protocolo} processada localmente.`);
    }
