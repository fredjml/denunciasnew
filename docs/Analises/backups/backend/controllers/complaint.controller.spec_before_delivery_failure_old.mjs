...
describe('receberDenuncia', () => {
  ...
  it('characterizes the current 201 response when the internal MPT API fails', async () => {
    process.env.MPT_API_URL = 'https://mpt.invalid/denuncias';
    ...
    expect(response.status).toHaveBeenCalledWith(201);
    expect(response.json).toHaveBeenCalledWith(expect.objectContaining({ sucesso: true }));
    expect(next).not.toHaveBeenCalled();
  });

  it('characterizes the current 201 response when MPT_API_URL is absent', async () => {
