...
describe('application perimeter', () => {
  ...
  it('refuses to start production without required configuration', () => {
    const app = { listen: vi.fn() };

    expect(() => startServer(app, createEnvironment({ NODE_ENV: 'production' }))).toThrow(
      'Configuração obrigatória ausente em produção: MPT_API_URL, FRONTEND_URL',
    );
    expect(app.listen).not.toHaveBeenCalled();
  });

  it('starts production when required configuration is present', () => {
    const server = { close: vi.fn() };
    const app = { listen: vi.fn().mockReturnValue(server) };
    const environment = createEnvironment({
      NODE_ENV: 'production',
      MPT_API_URL: 'https://api.interna.mpt.invalid/denuncias',
      FRONTEND_URL: 'https://denuncias.mpt.mp.br',
      PORT: '3000',
    });

    expect(startServer(app, environment)).toBe(server);
